...
export default function EnquiriesPage() {
  ...
  useEffect(() => {
    fetchEnquiries()
    resetNewEnquiryCount() // Clear notification count when viewing enquiries
  }, [])
  ...
  const updateStatus = async (enquiryId: number, newStatus: EnquiryStatus) => {
    try {
      ...
      if (response.ok) {
        ...
        const oldEnquiry = enquiries.find(e => e.id === enquiryId)
        if (oldEnquiry?.status === "NEW" && newStatus !== "NEW") {
          // In a real app, you'd want to call an API to update this server-side
