    ...
    <div>
        ...
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className={
                isActive
                  ? "rounded-full border border-[#6F8494] bg-[#6F8494] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-300 hover:bg-[#5C7282] hover:shadow-md"
                  : "rounded-full border border-[#D7E0E5] bg-white px-5 py-2.5 text-sm font-medium text-[#53616A] shadow-sm transition-all duration-300 hover:border-[#9BAFBB] hover:bg-[#EAF0F4] hover:text-[#3E505B]"
              }
            >
              {category}
            </button>
      ...
      {filteredItems.length > 0 ? (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          {filteredItems.map((menuItem) => (
            ...
            >
              ...
              {menuItem.isPopular && (
                <div
                  className="absolute left-3 top-3 z-10 flex items-center gap-1.5 rounded-full bg-[#C28A4A] px-3 py-1.5 text-xs font-semibold text-white shadow-md"
                >
              ...
              <div className="flex flex-col p-5">
                ...
                {/* CATEGORY */}
                {menuItem.category && (
                  <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.15em] text-[#8096A3]">
                    {menuItem.category}
                  </p>
                )}
                ...
                <div className="mb-3 flex items-start justify-between gap-3">
                  <h3 className="line-clamp-2 text-lg font-semibold leading-snug text-[#292F33]">
                    {menuItem.title}
                  </h3>
                  <span className="shrink-0 whitespace-nowrap rounded-full bg-[#F3E9DE] px-3 py-1 text-sm font-bold text-[#7A4E2D]">
                    {menuItem.price}
                  </span>
                </div>
                ...
                {menuItem.description && (
                  <p className="mb-3 line-clamp-3 text-sm leading-6 text-[#737D83]">
                    {menuItem.description}
                  </p>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 rounded-xl border border-[#6F8494] bg-[#6F8494] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-300 hover:bg-[#5C7282] hover:shadow-md"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 rounded-xl border border-[#7A4E2D] bg-[#7A4E2D] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-300 hover:bg-[#6A4225] hover:shadow-md"
                  >
                    Order Now
                  </button>
                  ...
                  {showSuccess?.itemId === menuItem.id && (
                    <div className="text-center text-xs font-semibold text-[#6F8494]">
                      {showSuccess.message}
                    </div>
                  )}
                </div>
        ...
        >
          ...
          <h3
            className="text-lg font-semibold text-[#292F33]"
          >
            No items found
          </h3>

          <p
            className="mt-2 text-sm text-[#737D83]"
          >
